    ...
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold text-slate-900">{isEditing ? 'Edit Challenge' : t('create')}</h1>
            <button 
                onClick={handleExit}
                className="p-2 bg-white border border-slate-200 rounded-full text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all shadow-sm"
                aria-label={t('cancel')}
            >
                <X size={20} />
            </button>
          ...
          {steps.map((s) => {
             ...
             return (
              <div key={s.id} className="flex flex-col items-center gap-2 bg-slate-50 px-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${active ? 'bg-brand-500 text-white' : 'bg-white border border-slate-300 text-slate-400'}`}>
                  <Icon size={18} />
                </div>
                <span className={`text-xs font-medium ${active ? 'text-brand-700' : 'text-slate-500'}`}>{s.title}</span>
              </div>
             );
          })}
        </div>
      ...
      <Card className="min-h-[400px] relative">
        ...
        {step === 1 && (
          <div className="space-y-4 animate-fadeIn">
            <h2 className="text-xl font-bold text-slate-900">{t('basics')}</h2>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">{t('challenge_name')}</label>
              <input 
                type="text" 
                className="w-full p-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none text-slate-900 text-base" 
            ...
            <div>
               <label className="block text-sm font-medium text-slate-700 mb-1">{t('max_players')}</label>
               <input 
                  type="number" 
                  className="w-full p-3 bg-white border border-slate-200 rounded-xl outline-none text-slate-900 text-base"
        ...
        {step === 2 && (
          <div className="space-y-6 animate-fadeIn">
            <div className="flex justify-between items-center">
               <h2 className="text-xl font-bold text-slate-900">{t('define_goals')}</h2>
               {!showGoalForm && (
                 <button 
                    onClick={() => {
                        resetGoalForm();
                    ...
                    className="bg-brand-50 text-brand-600 px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 hover:bg-brand-100 transition-colors text-base"
                  >
            ...
            {showGoalForm && (
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 space-y-5 shadow-sm">
                 <div className="flex justify-between items-center border-b border-slate-200 pb-4">
                    <h3 className="font-bold text-slate-800">{editingGoalId ? 'Edit Goal' : t('add_goal')}</h3>
                    <button onClick={resetGoalForm} className="text-slate-400 hover:text-slate-600">
                      <X size={20} />
                    </button>
                 </div>
                 ...
                 <div className="space-y-4">
                   <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t('title')}</label>
                      <input 
                          type="text" 
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          type="number" 
                          className={`w-full p-3 rounded-xl border bg-white text-slate-900 outline-none text-base transition-colors ${
                            (newGoal.points || 0) < 0 ? 'border-rose-200 focus:ring-rose-500 focus:border-rose-500' : 'border-slate-200 focus:ring-brand-500'
                          }`}
                   ...
                   {newGoal.frequency !== 'once' && (
                     <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t('max_completions')}</label>
                        <input 
                          type="number" 
                          min="1"
                          className="w-full p-3 rounded-xl border border-slate-200 bg-white text-slate-900 outline-none text-base"
                   ...
                   {/* Icon Picker Upgrade */}
                   <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('icon')}</label>
                      <div className="relative mb-3">
                        <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                        <input 
                          type="text" 
                          placeholder="Search 100 icons..." 
                          className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 bg-white text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all"
                          value={iconSearch}
                          onChange={e => setIconSearch(e.target.value)}
                        />
                      </div>
                      <div className="grid grid-cols-6 sm:grid-cols-8 gap-2 h-44 overflow-y-auto p-3 bg-white rounded-xl border border-slate-200 custom-scrollbar overscroll-contain">
                        {filteredIcons.map(k => {
                            ...
                            <button
                              ...
                              className={`aspect-square rounded-lg flex items-center justify-center transition-all active:scale-90 ${
                                isSelected 
                                  ...
                                  : 'bg-slate-50 text-slate-400 hover:bg-brand-50 hover:text-brand-500'
                              }`}
                              title={k}
                            >
                              <IconComp size={20} />
                 ...
                 <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                   <button type="button" onClick={resetGoalForm} className="px-4 py-2 text-slate-600 font-medium hover:bg-slate-100 rounded-lg text-base">{t('cancel')}</button>
            ...
            <div className="space-y-3">
              {formData.goals?.length === 0 && !showGoalForm && (
                <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl">
                  <p className="text-slate-400 mb-4">{t('no_goals')}</p>
              ...
              {formData.goals?.map(g => {
                  ...
                  return (
                    <div key={g.id} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl shadow-sm">
                       <div className="flex items-center gap-4">
                         <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${g.points < 0 ? 'bg-rose-50 text-rose-600' : 'bg-brand-50 text-brand-600'}`}>
                            <GoalIcon size={20} />
                         </div>
                         <div>
                           <h4 className="font-bold text-slate-900">{g.title}</h4>
                           <div className="text-xs font-bold">
                              <span className={g.points < 0 ? 'text-rose-500' : 'text-emerald-600'}>
                                  {g.points >= 0 ? '+' : ''}{g.points} pts
                              </span> 
                              <span className="text-slate-400 mx-1">•</span>
                              <span className="text-slate-500 lowercase">{t(g.frequency as any)}</span>
                           </div>
                         </div>
                       </div>
        ...
        {step === 3 && (
            <div className="space-y-6 animate-fadeIn">
                <h2 className="text-xl font-bold text-slate-900">{t('theme')}</h2>
                <div className="grid grid-cols-2 gap-4">
                    {THEME_IMAGES.map((theme, i) => (
                        <div 
                            ...
                            className={`relative rounded-2xl overflow-hidden cursor-pointer h-32 group ${
                                formData.coverImage === theme.url ? 'ring-4 ring-brand-500 ring-offset-2' : ''
                            }`}
                        >
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-slate-100">
          <button 
            ...
            onClick={handleBack} 
            disabled={step === 1}
            className={`px-6 py-2 rounded-xl font-medium flex items-center gap-2 text-base ${step === 1 ? 'opacity-0 pointer-events-none' : 'text-slate-600 hover:bg-slate-50'}`}
          >
          ...
          {step < 4 ? (
            <button 
              ...
              className="px-6 py-2 bg-slate-900 text-white rounded-xl font-medium hover:bg-slate-800 flex items-center gap-2 disabled:opacity-50 text-base"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
           <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl scale-100 transition-transform">
              <div className="flex flex-col items-center text-center mb-6">
                 ...
                 <h3 className="text-xl font-bold text-slate-900 mb-2">{t('confirm_exit_title')}</h3>
                 <p className="text-slate-500 text-sm">{t('confirm_exit_body')}</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                 <button 
                    onClick={() => setShowExitModal(false)}
                    className="py-3 px-4 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors text-base"
                 >
                    {t('keep_editing')}
                 </button>
                 <button 
